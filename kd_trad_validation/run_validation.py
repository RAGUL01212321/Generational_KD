#!/usr/bin/env python3
"""
Comprehensive Validation Script for Traditional KD
...
# Add parent directory and kd_traditional directory to path
sys.path.insert(0, str(Path(__file__).parent.parent))
sys.path.insert(0, str(Path(__file__).parent.parent / "kd_traditional"))
...
class ValidationPipeline:
    ...
    def run_quantitative_evaluation(self) -> Dict:
        ...
        kd_improvement_pct = (kd_improvement / base_metrics['kd_loss']) * 100 if base_metrics['kd_loss'] != 0 else 0
        
        ce_improvement = base_metrics['ce_loss'] - distilled_metrics['ce_loss']
        ce_improvement_pct = (ce_improvement / base_metrics['ce_loss']) * 100 if base_metrics['ce_loss'] != 0 else 0
    ...
    def run_qualitative_evaluation(self) -> Dict:
        ...
        samples = []
        for item in self.val_dataset.samples[:self.config.num_qualitative_samples]:
...
def main():
    parser = argparse.ArgumentParser(
        description="Validate Traditional Knowledge Distillation Pipeline"
    )
    ...
    if not val_path.exists():
        parent_val = Path("..") / args.validation_file
        if parent_val.exists():
            args.validation_file = str(parent_val)
            
    checkpoint_path = Path(args.distilled_checkpoint)
    if not checkpoint_path.exists():
        parent_ckpt = Path("..") / args.distilled_checkpoint
        if parent_ckpt.exists():
            args.distilled_checkpoint = str(parent_ckpt)
    ...
    results = pipeline.run_full_validation(run_qualitative=args.with_qualitative)
